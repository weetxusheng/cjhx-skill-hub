...
const recentSkillColumns = [
  ...
  { title: "分类", dataIndex: "category_name" },
  {
    ...
    render: (value: string | null) => (
      <Tag color={value ? reviewStatusTagColor(value) : "default"}>{formatReviewStatusLabel(value)}</Tag>
    ),
  },
  {
    title: "待办",
...
export function DashboardPage() {
  /**
   * 交互约定（工作台首页）：
   * - 分区按权限渲染：无权限时展示“无权限说明”，而非空白块。
   * - 各卡片查询独立：任一查询失败不会阻断其他模块渲染。
   * - 空态明确：列表为空时展示 Empty，避免“看起来像加载失败”。
   *
   * 数据口径说明：
   * - 待审核/待发布统计来自对应队列接口长度；
   * - 最近更新技能、最近处理记录、分类概览均展示受权限控制的可见子集。
   * - 顶部四格统计卡片可点击：主档数/已发布技能 → `/skills`，待审核 → `/reviews`，待发布 → `/releases`（无对应权限时不跳转）。
   */
    ...
    <>
      <Row gutter={[16, 16]}>
        <Col xs={24} md={12} xl={6}>
          <Card
            id="admin-dashboard-stat-skill-total"
            hoverable={canViewSkills}
            styles={{ body: { cursor: canViewSkills ? "pointer" : "default" } }}
            onClick={() => {
              if (canViewSkills) {
                navigate("/skills");
              }
        ...
        <Col xs={24} md={12} xl={6}>
          <Card
            id="admin-dashboard-stat-public-skills"
            hoverable={canViewSkills}
            styles={{ body: { cursor: canViewSkills ? "pointer" : "default" } }}
            onClick={() => {
              if (canViewSkills) {
        ...
        <Col xs={24} md={12} xl={6}>
          <Card
            id="admin-dashboard-stat-pending-reviews"
            hoverable={canReview}
            styles={{ body: { cursor: canReview ? "pointer" : "default" } }}
            onClick={() => {
              if (canReview) {
                navigate("/reviews");
              }
        ...
        <Col xs={24} md={12} xl={6}>
          <Card
            id="admin-dashboard-stat-pending-releases"
            hoverable={canPublish}
            styles={{ body: { cursor: canPublish ? "pointer" : "default" } }}
            onClick={() => {
              if (canPublish) {
                navigate("/releases");
              }
                ...
                <div id="admin-dashboard-pending-reviews-table-container">
                  <Table
                    ...
                    columns={[
                      {
                        title: "技能",
                        render: (_: unknown, record: ReviewQueueItem) => <Link to={`/versions/${record.version_id}`}>{record.skill_name}</Link>,
                      },
                ...
                <div id="admin-dashboard-pending-releases-table-container">
                  <Table
                    ...
                    columns={[
                      {
                        title: "技能",
                        render: (_: unknown, record: PendingReleaseItem) => <Link to={`/versions/${record.version_id}`}>{record.skill_name}</Link>,
                      },
                      { title: "版本", dataIndex: "version" },
                      {
                        title: "通过时间",
