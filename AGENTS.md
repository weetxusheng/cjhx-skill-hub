...
6. `docs/20-engineering/doc-update-matrix.md`
7. 与当前任务相关的 `docs/20-engineering/` 和 `docs/50-prompts/`

### 做功能开发前
1. `docs/10-architecture/repository-structure.md`
2. 对应技术栈的开发规范
3. 对应产品流文档
4. `docs/skill-hub-master-plan.md` 中当前阶段和 Next Task

### 做代码评审前
1. `docs/20-engineering/code-review-guide.md`
2. 当前变更涉及的架构与流程文档
3. `docs/20-engineering/doc-update-matrix.md`

### 做测试或发布前
1. `docs/20-engineering/api-and-testing.md`
2. `docs/20-engineering/feature-test-playbook.md`
3. `docs/40-runbooks/local-development.md`
4. `docs/40-runbooks/release-and-recovery.md`
5. `docs/deploy/`
...
- 做任何功能开发、修复、重构后，进入收尾阶段前必须读取并执行 `docs/20-engineering/feature-test-playbook.md`。
...
## What To Update When
详见 `docs/20-engineering/doc-update-matrix.md`。这里给出最常见映射：
...
- 改仓库结构：同步 `docs/10-architecture/repository-structure.md`
- 改 API：同步 `docs/20-engineering/api-and-testing.md`
- 改测试门禁或回归流程：同步 `docs/20-engineering/api-and-testing.md`、`docs/20-engineering/feature-test-playbook.md`、`docs/50-prompts/qa-guide.md`、`AGENTS.md`
...
1. 先确认任务属于哪一类：
   ...
   - 运维 / 发布 / 恢复
2. 按任务类型读取一级事实来源文档
3. 再读取对应 `docs/20-engineering/` 规范
4. 最后才参考 `docs/50-prompts/` 和 `docs/skill-hub-master-plan.md`

### 做实现时
- 先按 `docs/20-engineering/doc-update-matrix.md` 判断要同步哪些文档、测试和 runbook
- 普通实现过程中保持项目在线，优先依赖热更新，不要把“停服务再启动”作为默认动作
- 每次代码改动完成后，先执行 `pnpm local:ensure`，再做手工验证、联调和测试收尾
- 功能完成后，必须按 `docs/20-engineering/feature-test-playbook.md` 判断测试层级并执行对应测试
- 若代码现状与文档冲突，先修正文档或实现，不允许“先记在脑子里”
- 若发现历史计划和当前实现冲突，以 `docs/10-architecture/`、`docs/30-product-flows/` 为准

### 做评审和发布时
- 评审先看 `docs/20-engineering/code-review-guide.md`
- 测试收尾先看 `docs/20-engineering/feature-test-playbook.md`
...
## Spec Lifecycle
- 规范补充机制详见 `docs/20-engineering/spec-lifecycle.md`
- 默认规则：
  - 改架构、权限、状态机、API、统计口径，先补或同步补文档
  - 改测试门禁、回归流程、交付完成定义，先补或同步补 `docs/20-engineering/feature-test-playbook.md`、`docs/50-prompts/qa-guide.md`、`AGENTS.md`
  - 文档未同步视为未完成变更
